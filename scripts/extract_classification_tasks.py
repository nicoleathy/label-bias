...
def prepare_classification_tasks_split(task_info_file, tasks_data_dir, split_output_path, verbose=False):
    ...
    # choose the final list of classification tasks
    chosen_tasks = list()
    ...
    for task in candidate_tasks:
        ...
        all_answer_choices = [example['output'][0].lower().strip() for example in task_data['Instances']]
        num_answer_choices = len(np.unique(all_answer_choices))
        if num_answer_choices >= 50:
            if verbose:
                print(f'Skipping task {task} -- too many answer choices')
            continue
        if np.mean(num_outputs) >= 2:
            if verbose:
                print(f'Skipping task {task} -- multiple expected outputs')
            continue
        if num_examples < MINIMAL_NUM_INSTANCES:
            if verbose:
                print(f'Skipping task {task} -- insufficient instances')
            continue
...
def edit_raw_task_data(
    ...
    if add_positive_examples:
        ...
        if n_instances < (MIN_NUM_EVAL_INSTANCES + NUM_POS_EXAMPLES):
            print(f'Skipping task: {os.path.basename(filename)} -- insufficient instances for positive examples')
            return "skip"
        labels = [example['output'][0].lower().strip() for example in raw_data['Instances']]
        pos_examples_idx, _ = train_test_split(
            np.arange(n_instances),
            stratify=labels,
            train_size=NUM_POS_EXAMPLES,
            random_state=42
        )
        ...
        for i, example in enumerate(new_pos_examples):
            if isinstance(example['output'], list):
                example['output'] = example['output'][0].lower().strip()
    ...
    if add_answer_choices:
        all_answer_choices = [example['output'][0].lower().strip() for example in raw_data['Instances']]
...
if __name__ == "__main__":
    ...
    prepare_classification_tasks_split(full_task_info, tasks_data_dir, split_output_path, verbose=True)
