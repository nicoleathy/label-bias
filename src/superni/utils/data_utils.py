...
def prepare_domain_content_free_inputs(args):
    ...
    def _flat_cf(task_texts, n):
        """Template-free fallback: bag-of-words over whole inputs.

        Used for tasks without a template, and as a safety net when a
        templated task's examples do not split consistently into parts.
        """
        texts_processed = [t.lower().split() for t in task_texts if t]
        if not texts_processed:
            return ["" for _ in range(n)]
        mean_len = int(np.ceil(np.mean([len(x) for x in texts_processed])))
        bag = list(chain(*texts_processed))
        out = []
        for _ in range(n):
            if mean_len == 0 or not bag:
                out.append("")
            else:
                out.append(" ".join(
                    np_rand_generator.choice(bag, mean_len, replace=False)))
        return out
    ...
    for task in list(dict.fromkeys(tasks.tolist())):
        ...
        if task in task_templates.index:
            ...
            # Robustness fix: in the full task set, some examples may split
            # into fewer parts than the template implies (template/instance
            # mismatch). The original code assumed every example had the same
            # part count as the first example, so per-part indexing (x[i])
            # overflowed on shorter examples (IndexError). Instead, take the
            # modal part count and keep only examples that conform; if the
            # template yields nothing usable, fall back to the flat path.
            part_counts = Counter(len(ex) for ex in text_parts_processed)
            if not part_counts:
                all_cf_strings.extend(_flat_cf(texts, num_cf_inputs))
                continue
            n_parts = part_counts.most_common(1)[0][0]
            conforming = [ex for ex in text_parts_processed if len(ex) == n_parts]
            if n_parts == 0 or not conforming:
                if len(part_counts) > 1:
                    print(f"[DC ragged] task={task} part-counts={dict(part_counts)} "
                          f"-> using flat fallback")
                all_cf_strings.extend(_flat_cf(texts, num_cf_inputs))
                continue
            if len(part_counts) > 1:
                dropped = len(text_parts_processed) - len(conforming)
                print(f"[DC ragged] task={task} part-counts={dict(part_counts)} "
                      f"-> kept {len(conforming)}, dropped {dropped} examples")
            text_parts_processed = conforming

            parts_idx = list(range(n_parts))
            ...
            # Bug fix: original referenced an undefined `num_cf_strings`,
            # which would raise NameError on any non-templated task. Route
            # through the same flat content-free generation as the fallback.
            all_cf_strings.extend(_flat_cf(texts, num_cf_inputs))
